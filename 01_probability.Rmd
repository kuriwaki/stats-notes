...
author:
- name: Shiro Kuriwaki
  affiliation:  API 201-Z Teaching Fellow
...
\noindent (These optional notes are only intended to be supplements to review session and Professor Saghafian's lectures. They emphasize important ideas and tries to connect them with verbal explanation and worked examples. However, they are not meant to be comprehensive, and they may contain my own errors, which I will fix as I find them. I rely on multiple sources for explanation and examples\footnote{DeGroot and Schervish (2012), Blitzstein and Huang (2014), Blitzstein and Morris (unpublished manuscript), Imai (2017), and Ross (2003)}. I'd appreciate any feedback on what types of such material would be helpful for you to learn the class material.)

# Where are we? Where are we going?
The first three weeks of the course cover probability, a fundamental concept in quantitative reasoning. Probability is especially important for policy makers and social scientists, who must make inferences and predictions about the social world. Probability has an intuitive sense to it, which is useful, but to think about random variables and distributions requires some formal definitions.

Probability and inference are mirror images of each other. After probability the class will move on to inference. After that, we move on to models  (namely linearly regression) for making inferences -- the credibility of these models rests on statements about the distribution of the estimators used. Throughout, probability is fundamental.
...
# A more formal definition of probability
...
In naive probability, often with using cards and die, we are used to writing $P(A_1 \cap A_2 \cap ... A_3)$ as the product $P(A_1)P(A_2)P(A_3)$. But we should be cautious in doing this switch -- this only holds when all events are independent together (and in the social world, not many things are).
...
In English, when you're told for example two events (e.g. two scandals) developed independently, the first image that might come to mind is that the two events are separate. However, this impression can be misleading!

\begin{error}[Indepence and Disjointness]
Independence and Disjointness are two different concepts. The former does not imply the latter and the latter does not imply the former.

In the simplest example, consider a single coin flip. There are two events, Heads or Tails. These are disjoint (both cannot happen at the same time) but they are clearly \emph{dependent}.If you know a flip is Heads, it is definitely not Tails!
\end{error}

The clearest and safest way to understand independence might be through again relying on conditioninal probability. Two events are independent if knowing information about one event does not \emph{add} or \emph{detract} information about another.
...
For a partition of $A$: $A_1, A_2, \cdots A_k$
\end{definition}

But the intermediary formualtions we derive from Bayes Rule from the definition of conditional probability are equally important
...
A Lie Detector gives one of two  results: ``Honest'' or ``Liar'' and is 80 percent accurate. That is, it detects Honest people correctly 80 percent of the time, and it dtects Liars correctly 80 percent of the time. Suppose 10 percent of the entire population are liars. Suppose we tested the lie detector on a representative sample of the population of 100 people. We then inspect closely the subset of people for whom the Lie Detector reported "Liar". Among this subset, what is the probability that any given member is truly a liar?
...
&\approx 0.30
\end{align*}
We could decompose the denominator in the above way because we know the events ``Liar'' and ``Honest'' are disjoint and parition the sample. Otherwise, we would not have been able to rewrite this probability in this way.
...
Final note: This example is a case in which the probabilities associated with an event is only expressable in a table. There is no one-line formula, at least at first glance, to get from the values of $X(s)$ to $P(X = x)$. The distributions that we will see in the next section have specific formulas.
...
\[f(x) = F^\prime(x)\]
where the prime symbol denotes derivation with respect to $X$.  The two equations will be equivalent due to the fundamental theorems of calculus.
\end{definition}
...
\tcbox{Answer} You \emph{could} compute $\sum_{x = 0, 1, ..., 30}{30 \choose x}(0.1)^{x}(0.9)^{30 - x}$. But the expecation of known r.v.'s such as the Binomial is derived for you, and in this case has an intuitive feel to it. $E(X) = np = 3$.

\medskip

\noindent 4. What is the probability that more than $27$ people arrive \emph{on time}?

\tcbox{Answer} That's the probability that either 0, 1, or 2 people arrive late. The probability of this union is the sum of probabilities, becaus events are disjoint. So
...
The interesting point about a Poisson is that $x$ is countably infinite. That is we can compute quanties like $P(X = 99999999)$ and keep on going. In a Binomial, we had to stop at $n$. If $X$ can take on infinitely many values and this probability table will go on for ever, you might doubt whether the PMF will sum to 1. After all, we are summing an infinite amount of things. However, it does not. The sum of this sequence actually converges to 1.
\end{example} \qed
...
Even though its PDF looks daunting, Normal distributions come up everywhere. In inference, we will start with the Central Limit Theorem, which posits that the sum of most random variables will be distrbuted as a Normal distribution as the number of elements increase. This means that many phenomena we observe in real life have approximately a Normal distribution. 

The densties of a Normal PDF vary by their parameters, i.e. $\mu$ and $\sigma^2$. However, a useful fact is that any Normal r.v. has a straightforward transformation to our favorite type of Normal, a standard Normal. To get this, subtract $\mu$ from $X$ and then divide that difference by $\sigma$.
...
\begin{error}[Cateogry Error for $E$]
...
\begin{error}[Cateogry Error for $\var$]
...
A sequence of random variables, $X_1, ... X_n$ is said to have the Markov property if for each of random varaibles, the ``future'' state (the subsequent $X_i$) depends only on the present state, i.e. is independent of past states conditional on the past states:
...
A sequence of random variables that have the Markov property is simply caled a Markov Chain.
\end{definition}

That is, we allow for one level of depdendency in a sequence of random variables. This is probably not enough to be a true representation of the world --- past events from varying time influence our current state --- but it is much better than the completely independent world we would have dealt with otherwise. 
...
That is, the values of the probablities in each column will eventually become the same number. Intuitively, after many turns of systematic status change, your original position at $t= 0$ becomes irrelevant. 

The probablities $\pi_1,... \pi_k$ is a quantity of interest; it quantitifies the probability that an observation in the Markov Chain ends up in a statuses $1, ...k$ after a number of changes. It is possible to compute what the values of these are just from the probability transition martrix (which contains information about one step).
...
From matrix algebra, this equation indicates a set of equalities, with two unknowns $(\pi_1, \pi_2)$ and know values $(P_{1,1}, P_{1,2}, P_{2,1}, P_{2,2})$
...
(c) Compute steady-state probabilities of a citizen being in the lower class($\pi_L$), in the middle class ($\pi_M$) or in the upper class ($\pi_U$). Remember that the steady-state probabilities sum to 1, $\pi_L + \pi_M + \pi_U = 1$. Yout  final answer should be in simple fractions.)
