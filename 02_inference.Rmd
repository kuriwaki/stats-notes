...
\epigraph{``We are so lucky that we live in a world where the Central Limit Theorem is true''}{My first stats professor in undergrad}

\noindent \large{$^\star$} \footnotesize{These optional notes are only intended to be supplements to review session and Professor Saghafian's lectures. They emphasize the important ideas and tries to connect them with verbal explanation and worked examples. However, they are not meant to be comprehensive, and they may contain my own errors, which I will fix as I find them. I rely on multiple sources for explanation and examples\footnote{DeGroot and Schervish (2012), Blitzstein and Huang (2014), Blitzstein and Morris (unpublished manuscript), Imai (2017), Diez, Barr, and Cetinkaya-Rundel (2015), Moore, McCabe, and Craig (2002)}. I'd appreciate any feedback on what types of such material would be helpful for you to learn the class material.} 
...
In a word where we only have data and no knowledge about the probability distribution or the nature of a random variable or events, we now must construct estimators. Estimators (1) take data and (2) apply one set of manipulations to that data.  
...
\tcbox{Answer} Because we had more sample in part (d), and the true mean happened to be different from our threshold of interest. We can simulate the distribution of $X_1$ and $\bar{X}_n$ (see figure). Because the sampling distribution with the larger $n$ had a smaller variance, there is much less area under the curve larger than 15 in that distribution. In other words, in distributions with smaller variance, values that are away from the mean become increasingly less likely to occur (almost by definition of variance).
...
```{r, echo = FALSE}
nsims <- 1000
df_sims <- data.frame(X = c(rnorm(nsims, mean = 13, sd = sqrt(16)), rnorm(nsims, mean = 13, sd = sqrt(16/40))),
                      n = c(rep(1, nsims), rep(40, nsims)))

ggplot(df_sims, aes(x = X)) + 
  facet_grid(n~., labeller = label_both) +
  geom_histogram(bins = 30) +
  geom_vline(xintercept = 15, linetype = "dashed") +
  labs(x = expression(bar(X)[n]),
       y = "",
       title = expression(X%~%Normal(mu == 13, sigma^2==16)))
...
\begin{tabularx}{X|cc}
& Lost Medicaid Lottery & Won Medicaid Lottery \\\hline
...
\includegraphics[width = \textwidth]{figures/turnout_state.pdf}
...
t2008 <- read_csv("data/2008_turnout.csv", skip = 1) %>% 
  slice(-1) %>%
  mutate(trn = `Highest Office` / `Voting-Eligible Population (VEP)`, 
         year = 2008,
         st = row.names(States)) %>% 
  arrange(trn) %>%
  select(st, trn, year)
...
ggplot(t_all, aes(x = forcats::fct_inorder(st), y = trn, shape = factor(year))) + 
  ...
  geom_point() +
  scale_shape_manual(values = c(1, 2), labels = c("2012 (Obama - Romney)", "2016 (Trump - Clinton)")) +
  guides(shape = guide_legend(title = "Year")) +
  labs(y = "Turnout (as percentage of VEP)",
       x = "State")
ggsave("figures/turnout_state.pdf", w = 12, h = 5)
```
...
converges to a Chi-squared test. The quantity of proportions is something quite different. The CLT is very general, but unfortunately this form is not exactly the CLT form as in Theorem \ref{thm:clt}.  Using this incorrect test statistic will give you ``a'' number for the test-statistic? but it will be incorrect to line that number up against a $\chi^2$ distribution table and table and conclude that the p-value is $P(\chi^2_{\textit{df}} > \text{test-stat})$ 
\end{error}


## Chis-squared tests for goodness of fit (one-way)
...
We can extend the theorem that the standardized counts converge to a Normal under the null in a two-way case. 
